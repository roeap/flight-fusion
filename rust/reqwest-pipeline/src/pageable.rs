...
pub struct Pageable<T> {
    stream: std::pin::Pin<Box<dyn Stream<Item = Result<T, crate::ReqwestPipelineError>>>>,
}

impl<T: Continuable> Pageable<T> {
    pub fn new<F>(make_request: impl Fn(Option<String>) -> F + Clone + 'static) -> Self
    where
        F: std::future::Future<Output = Result<T, crate::ReqwestPipelineError>> + 'static,
    {
...
impl<T> Stream for Pageable<T> {
    type Item = Result<T, crate::ReqwestPipelineError>;
