...
use arrow_deps::datafusion_data_access::object_store::{
    FileMetaStream, ListEntryStream, ObjectReader, ObjectReaderStream, ObjectStore,
};
use arrow_deps::datafusion_data_access::{FileMeta, Result, SizedFile};
...
impl ObjectReader for LocalFileReader {
    async fn chunk_reader(&self, start: u64, length: usize) -> Result<Box<dyn AsyncRead>> {
        let mut file = tokio::fs::File::open(&self.file.path).await?;
        file.seek(SeekFrom::Start(start)).await?;
        let file = TokioBufReader::new(file.take(length as u64));
