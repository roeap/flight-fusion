...
def create_definition_ref(definition: str, version: str = SupportedKubernetes.V1_18) -> str:
    return f"https://kubernetesjsonschema.dev/v{version}/_definitions.json#/definitions/{definition}"


def create_json_schema_conditionals(enum_type_to_config_name_mapping: Dict[Enum, str]) -> List[dict]:
    return [
        {
            "if": {
