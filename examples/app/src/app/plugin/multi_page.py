...
def register_page(
    ...
    ***
    ...
    if mod is None:
        raise ImportError("register page must be called from within module")
    module = mod.__name__
...
def _import_layouts_from_pages(pages_folder, module=None):
    for (root, dirs, files) in os.walk(pages_folder):
        for file in files:
            ...
            if module is None:
                import_key = f"pages.{page_filename}"
            else:
                import_key = f"{module}.pages.{page_filename}"
...
def plug(app):  # noqa: C901
    ...
    if os.path.exists(pages_folder):
        if mod is not None:
            _import_layouts_from_pages(pages_folder, mod.__name__)
        else:
