...
def string_maker():
    class Dummy:
        def __call__(self, size=5):
            return "".join(random.choice(string.ascii_letters) for _ in range(size))  # nosec
...
@pytest.mark.parametrize("experiment", [None, MagicMock(experiment_id="1"), MagicMock(experiment_id="lol")])
...
def test_setup(mock_atexit, context):
    ...
    ) as mock_get_current_run_id, patch.object(MlFlow, "_set_active_run") as mock_set_active_run, patch.object(
    ...
    mock_atexit.assert_called_once_with(mlf.end_run)  # type: ignore
...
def test_set_active_run_parent_zero(child_context):
    ...
    with patch.object(MlFlow, "_start_run") as mock_start_run:
        ...
        mock_start_run.assert_any_call(run_id="what-is-an-edge-case", run_name=mlf.run_name, nested=True)
...
def test_execute_solid_with_mlflow_resource():
    ...
    def solid1(_):
        ...
        run_id_holder["solid1_run_id"] = mlflow.active_run().info.run_id  # type: ignore
